...
function NativeAddAccountButton({
  linkToken,
  setLinkToken,
  createLinkToken,
  onPlaidSuccess,
  ...
  const fetchAccounts = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/plaid/accounts`,
        config,
      );
  ...
  const onPlaidSuccess = useCallback(async (publicTokenOrSuccess, metadata) => {
    ...
    try {
      ...
      await axios.post(
        ...
        { publicToken, firebaseUuid: user.uid, institutionName, institutionId },
        config,
      );
