...
export default function PaycheckSavingsScreen({ navigation, route }) {
  ...
  // Total credit card debt from Plaid snapshot at onboarding time.
  // null = unknown/not captured; 0 = genuinely no debt. Preserve null as-is.
  ...
  const handleFinalize = async () => {
    ...
    if (
      ...
      Alert.alert(
        "Invalid Pay Day",
        "Please enter valid days of the month (1-31).",
      );
      setIsSaving(false);
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/budget/finalize`,
        {
          ...
          // Persists the Plaid debt snapshot captured at onboarding time.
      ...
      Alert.alert(
        "Error Finalizing Budget",
        e.message || "An unknown API error occurred.",
      );
    }
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <SafeAreaView style={styles.container}>
        ...
        <Text style={styles.label}>
          1. What is your NET (take-home) paycheck amount?
        </Text>
        <TextInput
        ...
        {debtPerPaycheck != null && (
          <Text style={styles.debtNote}>
            Debt payoff included: ${parseFloat(debtPerPaycheck).toFixed(2)} /
            paycheck
          </Text>
        )}
