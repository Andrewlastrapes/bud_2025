...
export default function SavingsOnboardingScreen({ navigation, route }) {
  ...
  const remainingAfterDebt =
    route.params?.remainingAfterDebt ?? paycheckAmount - debtPerPaycheck;
    ...
    try {
      ...
      navigation.navigate("DynamicAmountFinal", {
        ...
        dynamicSpendableAmount:
          data.dynamicSpendableAmount ?? data.remainingToSpend,
  ...
  const handleContinue = () => {
    ...
    if (isNaN(v) || v < 0) {
      setInputError("Enter a valid amount or leave blank for $0.");
      return;
    }
    handleFinalize(v);
  };
  ...
  const finalRemaining =
    Math.round((remainingAfterDebt - savingsValue) * 100) / 100;
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      <SafeAreaView style={s.safe}>
        ...
        <ScrollView contentContainerStyle={s.scroll}>
          ...
          <Text style={s.sub}>
            Set aside money each paycheck before you spend it.
          </Text>
          ...
          <View style={s.availCard}>
            <Text style={s.availLabel}>
              {hasDebt
                ? "After debt payment, you have"
                : "After fixed costs, you have"}
            </Text>
            <Text
              style={[
                s.availAmt,
                remainingAfterDebt < 0 && { color: C.danger },
              ]}
            >
            ...
            <View style={s.warnCard}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={s.warnBody}>
                  ...
                  /paycheck toward debt. Consider pausing savings until your
                  debt is reduced — but the choice is yours.
                </Text>
              </View>
            </View>
          )}
          ...
          <Text style={s.hint}>
            Leave blank or $0 to skip. You can change this anytime.
          </Text>
          ...
          <View style={s.previewCard}>
            ...
            <Divider
              style={{ marginVertical: 10, backgroundColor: "#E5E7EB" }}
            />

            <View style={s.row}>
              <Text style={s.pl}>Income</Text>
              <Text style={[s.pv, { color: C.success }]}>
                +${paycheckAmount.toFixed(2)}
              </Text>
            </View>
            {fixedCostsRemaining > 0 && (
              <View style={s.row}>
                <Text style={s.pl}>Fixed costs</Text>
            ...
            {hasDebt && (
              <View style={s.row}>
                <Text style={s.pl}>Debt payoff</Text>
                <Text style={[s.pv, { color: C.danger }]}>
                  −${debtPerPaycheck.toFixed(2)}
                </Text>
              </View>
            ...
            <View style={s.row}>
              <Text style={[s.pl, { fontWeight: "700", color: C.text }]}>
                Remaining to spend
              </Text>
              <Text
                style={[
                  s.pv,
                  {
                    fontWeight: "800",
                    fontSize: 16,
                    color: finalRemaining >= 0 ? C.success : C.danger,
                  },
                ]}
              >
