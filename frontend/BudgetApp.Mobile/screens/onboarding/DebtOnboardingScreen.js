  ...
  return [0.1, 0.2, 0.3].map((apr) => {
    ...
    if (monthlyPayment > 0) {
      ...
      return {
        apr: apr * 100,
        months,
        totalInterest: Math.max(0, months * monthlyPayment - principal),
      };
    }
    ...
    return {
      apr: apr * 100,
      months: n,
      totalInterest: Math.max(0, n * pmt - principal),
    };
  });
}
...
export default function DebtOnboardingScreen({ navigation, route }) {
        ...
        const p2 = axios
          .post(
            `${API_BASE_URL}/api/budget/base`,
            {
              ...
              payDay1,
              payDay2,
              nextPaycheckDate: calcNextPaycheck(payDay1, payDay2),
            },
            config,
          )
  ...
  const goNext = (debt) =>
    navigation.navigate("SavingsOnboarding", {
      ...
      // Pass the Plaid snapshot total so it can be persisted at finalization.
  ...
  if (!snapshot || totalDebt <= 0) {
    return (
      <View style={[s.center, { padding: 32 }]}>
        <View style={s.noDebtIcon}>
          <Text style={{ fontSize: 32 }}>✅</Text>
        </View>
        <Text style={s.noDebtTitle}>No debt detected</Text>
        <Text style={s.noDebtBody}>
          We didn't find any outstanding credit card balances in your linked
          accounts.
        </Text>
        <Button
          mode="contained"
  ...
  return (
    ...
    >
      ...
      <Text style={s.sub}>
        Choose how much to put toward debt each paycheck.
      </Text>
      ...
      <View style={s.availCard}>
        ...
        <Text style={s.availNote}>
          ${paycheckAmount.toFixed(2)} paycheck − $
          {(baseBudget?.fixedCostsRemaining ?? 0).toFixed(2)} fixed costs
        </Text>
      </View>
      ...
      {canPayInFull && (
        <View style={s.fullCard}>
          <Text style={s.fullTitle}>
            🎉 You can clear all debt this paycheck!
          </Text>
          <Text style={s.fullBody}>
            Your available budget (${baseRemaining.toFixed(2)}) covers your
      ...
      <View style={s.presetRow}>
        {PRESETS.map((amt) => (
          ...
          >
            <Text
              style={[s.presetTxt, selectedPreset === amt && s.presetTxtActive]}
            >
              ${amt}
            </Text>
          </TouchableOpacity>
        ...
        <View style={s.timelineCard}>
          <Text style={s.timelineTxt}>
            ...
            <Text style={{ fontWeight: "700" }}>
              ${selectedAmount.toFixed(2)}/paycheck
            </Text>{" "}
            pays off{" "}
            <Text style={{ fontWeight: "700" }}>${totalDebt.toFixed(2)}</Text>{" "}
            in{" "}
            <Text style={{ fontWeight: "700", color: C.primary }}>
              {numPaychecks} {numPaychecks === 1 ? "paycheck" : "paychecks"}
            </Text>
            .
          </Text>
        </View>
      ...
      {selectedAmount > 0 && (
        <View
          style={[s.previewCard, remainAfterDebt < 0 && s.previewCardDanger]}
        >
          <Text style={s.previewTitle}>After debt payment</Text>
          <Divider style={{ marginVertical: 10, backgroundColor: "#E5E7EB" }} />
          <View style={s.previewRow}>
            <Text style={s.pl}>Available</Text>
            <Text style={s.pv}>${baseRemaining.toFixed(2)}</Text>
          </View>
          <View style={s.previewRow}>
            <Text style={s.pl}>Debt</Text>
            <Text style={[s.pv, { color: C.danger }]}>
          ...
          <View style={s.previewRow}>
            ...
            <Text
              style={[s.pv, s.bold, remainAfterDebt < 0 && { color: C.danger }]}
            >
              ${remainAfterDebt.toFixed(2)}
            </Text>
          </View>
          {remainAfterDebt < 0 && (
            <Text style={s.warnTxt}>⚠️ Amount exceeds available budget.</Text>
          )}
        </View>
      )}
